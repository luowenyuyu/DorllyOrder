...
{
    public partial class ContractRefund_WPPT : AbstractPmPage, System.Web.UI.ICallbackEventHandler
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (hc != null)
                {
                    ...
                    CheckRight(user.Entity, "pm/Op/ContractRefund_WPPT.aspx");
                    ...
                    if (!Page.IsCallback)
                    {
                        if (user.Entity.UserType.ToUpper() != "ADMIN")
                        {
                            string sqlstr = "select a.RightCode from Sys_UserRight a left join sys_menu b on a.MenuId=b.MenuID " +
                                "where a.UserType='" + user.Entity.UserType + "' and menupath='pm/Op/ContractRefund_WPPT.aspx'";
            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ResourceService.ResourceService srv = new ResourceService.ResourceService { Timeout = 5000, Url = ConfigurationManager.AppSettings["ResourceServiceUrl"].ToString() };
                            string Items = "";
