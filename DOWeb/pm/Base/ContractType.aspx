<%@ Page Language="C#" AutoEventWireup="true" Inherits="project.Presentation.Base.ContractType,project"  %>
<!DOCTYPE>
...
<body>
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 基础资料 <span class="c-gray en">&gt;</span> 合同类型 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:2px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    ...
    <div id="edit" class="pt-5 pr-20 pb-5 pl-20" style="display:none;">
      <div class="form form-horizontal bk-gray mt-15 pb-10" id="editlist">
        <div class="row cl">
          <label class="form-label col-2"><span class="c-red">*</span>合同类型编号：</label>
          <div class="formControls col-4">
            <input type="text" class="input-text required" placeholder="合同类型编号" id="ContractTypeNo" data-valid="isNonEmpty||between:1-16" data-error="合同类型编号不能为空||合同类型编号长度为1-16位" />
          </div>
            <div class="col-3"></div>
        </div>
        <div class="row cl">
          <label class="form-label col-2"><span class="c-red">*</span>合同类型名称：</label>
          <div class="formControls col-4">
            <input type="text" class="input-text required" placeholder="合同类型名称" id="ContractTypeName" data-valid="isNonEmpty||between:2-50" data-error="合同类型名称不能为空||合同类型名称长度为2-50"/>
          </div>
          <div class="col-3"></div>
        </div>
        <div class="row cl">
    ...
    <script type="text/javascript">
        function BandResuleData(temp) {
            ...
            if (vjson.type == "submit") {
                if (vjson.flag == "1") {
                    ...
                    showMsg("ContractTypeNo", "此合同类型编号在系统已存在", "1");
                }
                else {
                    layer.msg("数据操作出错！");
                }
            ...
            if (vjson.type == "update") {
                if (vjson.flag == "1") {
                    $("#ContractTypeNo").val(vjson.ContractTypeNo);
                    $("#ContractTypeName").val(vjson.ContractTypeName);

                    $("#ContractTypeNo").attr("disabled", true);
        ...
        function insert() {
            ...
            $("#ContractTypeNo").attr("disabled", false);
            $("#ContractTypeNo").val("");
            $("#ContractTypeName").val("");
        ...
        function submit() {
            if ($('#editlist').validate('submitValidate')) {
                ...
                submitData.id = id;
                submitData.ContractTypeNo = $("#ContractTypeNo").val();
                submitData.ContractTypeName = $("#ContractTypeName").val();
