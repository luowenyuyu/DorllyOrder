<%@ Page Language="C#" AutoEventWireup="true" Inherits="project.Presentation.Op.CRDisposit,project"  %>
<!DOCTYPE>
...
<body>
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单管理 <span class="c-gray en">&gt;</span> 会议室退押金 <a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:2px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
    <div id="list" class="pt-5 pr-20 pb-5 pl-20">
	    ...
	    <div class="cl pd-10  bk-gray mt-2"> 
		    租用日期&nbsp;<input type="text" class="input-text size-MINI" style="width:150px" id="MinDate" />&nbsp;
            至&nbsp;<input type="text" class="input-text size-MINI" style="width:150px" id="MaxDate" />&nbsp;
            状态&nbsp;<select class="input-text size-MINI" style="width:120px" id="PayState">
                <option value="" >全部</option>
                <option value="2">客户申请退款</option>
                <option value="0" selected>客服申请退款</option>
    ...
    <script type="text/javascript">
        ...
        function refund() {
            layer.confirm('确认要退押金吗？', function (index) {
                var submitData = new Object();
                submitData.Type = "refund";
                submitData.id = $("#selectKey").val();

                submitData.MinDate = $("#MinDate").val();
                submitData.MaxDate = $("#MaxDate").val();
                submitData.PayState = $("#PayState").val();
        ...
        jQuery(function () {
            var MinDate = new JsInputDate("MinDate");
            var MaxDate = new JsInputDate("MaxDate");
        });
